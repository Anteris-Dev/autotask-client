 ...
 */
class AttachmentNestedAttachmentQueryBuilder
{
     ...
     */
     ...
     {
         $response = $this->client->get("AttachmentNestedAttachments/query/count", [
     ...
     * Runs the query.
     */
    public function get(): AttachmentNestedAttachmentCollection
    {
        $response = $this->client->get("AttachmentNestedAttachments/query", [
            'search' => json_encode( $this->toArray() )
        ]);

        return AttachmentNestedAttachmentCollection::fromResponse($response);
    }

    /**
     * Runs the query and returns a paginator object.
     */
    public function paginate(): AttachmentNestedAttachmentPaginator
    {
        $response = $this->client->get("AttachmentNestedAttachments/query", [
            'search' => json_encode($this->toArray())
        ]);

        return new AttachmentNestedAttachmentPaginator($this->client, $response);
    }
    ...
    public function __toString()
    {
        $uri = UriResolver::resolve(
            ...
            new Uri('AttachmentNestedAttachments/query')
        );
